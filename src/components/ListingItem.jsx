...
function ListingItem({listing, id, onEdit, onDelete}) {
  return (
    <li className='relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-lg rounded-md overflow-hidden transition-shadow duration-150 m-[10px]'>
        ...
        <img className='h-[170px] w-full object-cover hover:scale-1050 transition-scale duration-200 ease-in' loading='lazy' src={listing.imgUrls[0]} alt="" />
        ...
        <div className='w-full p-[10px]'>
            ...
            .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
             ...
             <div className='flex items-center mt-[10px] space-x-3'>
                ...
                <div>
                    <p className='font-bold text-xs'>{listing.bathroom > 1 ? `${listing.bathrooms} Baths` : "1 Bath"}</p>
                </div>
             </div>
        </div>
